    ...
    <group ref={ref} position={position}>
      ...
      <Html distanceFactor={8} position={[0, 0.12, 0]} center>
        <div style={{
          ...
          {isPrimary ? "PRIMARY OBJECT" : "SECONDARY OBJECT"}<br/><span style={{ fontSize: "10px", color: "#fff" }}>{label}</span>
        </div>
      </Html>
    </group>
  );
...
function ConjunctionMarker({ position }: { position: [number, number, number] }) {
  ...
  return (
    <group position={position}>
      ...
      <mesh scale={2.2}>
        <sphereGeometry args={[0.07, 16, 16]} />
        <meshBasicMaterial color="#ff3333" transparent opacity={0.1} />
      </mesh>
...
function Scene({ selectedEvent, eventDetail }: SceneProps) {
  ...
  return (
    <>
      ...
      {orbitParams && (
        <>
          <OrbitPath inclination={orbitParams.inc1} radius={orbitParams.radius1} color="#00e5ff" opacity={0.2} />
          <OrbitPath inclination={orbitParams.inc2} radius={orbitParams.radius2} color="#d4af37" opacity={0.2} />
