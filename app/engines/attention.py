...
def _hours(tca):
    ...
    h=(tca-datetime.now(timezone.utc)).total_seconds()/3600
    # Past-TCA events are as time-critical as h==0; clamp to avoid silently skipping them
    return max(h, 0.0)
def uncertainty(u):
...
def assess(u: UpdateInput, escalation=False):
    ...
    if h<=24: add("time_to_tca",10,round(h,1),24,"Time remaining until closest approach is short.","hours")
